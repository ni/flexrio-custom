...
-- Purpose:
--
-- This file instantiates the DMA FIFOs that connect to the Chinch LvFpga
-- interface.  The FIFO instantiation is based on the DMA channel configuration
-- in the dynamically generated PkgCommIntConfiguration package.
--
-- Bogdan Popa - 09/09/2013
-- Added support for the Handshaking interface for FIFOs.
--
-- Harmish - 08/04/2014 - Added support for Flush method for FIFO.
-- * vFlushIn/Out/Clear signals create the enable chain for Flush method. 
--   These signals come from the interface.vhd and are hooked up to DmaPortCommIfcInputFifoInterface.
-------------------------------------------------------------------------------
...
entity DmaPortCommIfcFifos is
  port(
    ...
    DmaHandshakingInArray  : in  DmaHandshakingArray_t;
    DmaHandshakingOutArray : out DmaHandshakingArray_t;

	-- Enable chain for the flush method
	DmaFlushEnableInArray    : in DmaEnableArray_t;  
	DmaFlushEnableClearArray : in DmaEnableArray_t;
    DmaFlushEnableOutArray   : out DmaEnableArray_t;       
			
    -- The DMA FIFO count values.
    DmaCtCountArray     : out DmaCountArray_t;

    -- The enable chain associated with the FIFO count.
    DmaCtEnableInArray       : in  DmaEnableArray_t;
    DmaCtEnableOutClearArray : in  DmaEnableArray_t;
    DmaCtEnableOutArray      : out DmaEnableArray_t;

    -- The enable chain for stream state in the DMA clock domain.
    DmaStreamStateEnableInArray : in DmaEnableArray_t := (others=>'0');
    DmaStreamStateEnableOutArray : out DmaEnableArray_t;
    DmaStreamStateEnableClearArray : in DmaEnableArray_t := (others=>'0');
    DmaStreamStateOutArray : out
      StreamStateValueArray_t(kNumberOfDmaChannels-1 downto 0);

    -- The enable chain for stream state in the default clock domain.
    dStreamStateEnableInArray : in DmaEnableArray_t := (others=>'0');
    dStreamStateEnableOutArray : out DmaEnableArray_t;
    dStreamStateEnableClearArray : in DmaEnableArray_t := (others=>'0');
    dStreamStateOutArray : out StreamStateValueArray_t(kNumberOfDmaChannels-1 downto 0);
    ...
    dCurrentStreamStateArray : out StreamStateValueArray_t
      (kNumberOfDmaChannels-1 downto 0);

    -- The enable chain for start, stop, and stop with flush requests.
    dStartRequestEnableInArray : in DmaEnableArray_t := (others=>'0');
    dStartRequestEnableOutArray : out DmaEnableArray_t;
    dStartRequestEnableClearArray : in DmaEnableArray_t := (others=>'0');

    dStopRequestEnableInArray : in DmaEnableArray_t := (others=>'0');
    dStopRequestEnableOutArray : out DmaEnableArray_t;
    dStopRequestEnableClearArray : in DmaEnableArray_t := (others=>'0');

    dStopWithFlushRequestEnableInArray : in DmaEnableArray_t := (others=>'0');
    dStopWithFlushRequestEnableOutArray : out DmaEnableArray_t;
    dStopWithFlushRequestEnableClearArray : in DmaEnableArray_t := (others=>'0');
    dStopWithFlushRequestTimeoutArray : in DmaTimeoutArray_t := (others=>(others=>'0'));
    dStopWithFlushRequestTimedOutArray : out DmaFlagArray_t;
...
end DmaPortCommIfcFifos;

architecture struct of DmaPortCommIfcFifos is
  
  type CountUnsignedArray_t is array(kNumberOfDmaChannels-1 downto 0) of
  ...
  constant kFifoDepthInDataBusWidthWords : DmaChannelConfArray_t(kDmaFifoConfArray'range)
  := GetFifoDepths(kDmaFifoConfArray);
  ...
  constant kFifoDataWidthArray : FifoDataWidthArray_t :=
    GetFifoDataWidth(kDmaFifoConfArray);
  ...
  begin
    ...
    DmaComponents: for i in 0 to kNumberOfDmaChannels - 1 generate
      ...
      DmaInput: if kDmaFifoConfArray(i).Mode = NiFpgaTargetToHost or
        ...
        -- input stream.
        assert Log2(kDmaFifoConfArray(i).FifoDepth+2) >
               Log2(kDmaFifoConfArray(i).FifoDepth+1)
          report "kFifoDepth should be 2**N-1 where N is an integer."
          severity failure;

        assert kDmaFifoConfArray(i).FifoDepth+1 > kNiDmaDataWidthInBytes
          report "kFifoDepth should be at least the number of samples that fit in two data bus words."
          severity failure;

        assert kDmaFifoConfArray(i).FifoDepth+1 > kDmaFifoConfArray(i).ElementsPerClockCycle
        ...
        --vhook_a kNumOfSamplesPerWrite kDmaFifoConfArray(i).ElementsPerClockCycle
        --vhook_a kScl kDmaFifoConfArray(i).SCL
        --vhook_a kCountScl kDmaFifoConfArray(i).CountSCL
        --vhook_a kSignExtend kDmaFifoConfArray(i).SignedData
        --vhook_a kPeerToPeer (kDmaFifoConfArray(i).Mode = NiFpgaPeerToPeerWriter)
        --vhook_a kFxpType kDmaFifoConfArray(i).FxpType
        --vhook_a kDisableOnFifoTimeout kDmaFifoConfArray(i).DisableOnFifoTimeout
        --vhook_a kViClkIsDefaultClk kDmaFifoConfArray(i).DmaClkIsDefaultClk
        --vhook_a kWriteUsingHandshaking kDmaFifoConfArray(i).InterfaceIsHandshaking
        ...
        DmaPortCommIfcInputFifoInterfacex: entity work.DmaPortCommIfcInputFifoInterface (structure)
          generic map (
            ...
            kNumOfSamplesPerWrite  => kDmaFifoConfArray(i).ElementsPerClockCycle,        
            kScl                   => kDmaFifoConfArray(i).SCL,                          
            kCountScl              => kDmaFifoConfArray(i).CountSCL,                     
            kSignExtend            => kDmaFifoConfArray(i).SignedData,                   
            kFxpType               => kDmaFifoConfArray(i).FxpType,                      
            kPeerToPeer            => (kDmaFifoConfArray(i).Mode = NiFpgaPeerToPeerWriter),
            kDisableOnFifoTimeout  => kDmaFifoConfArray(i).DisableOnFifoTimeout,         
            kViClkIsDefaultClk     => kDmaFifoConfArray(i).DmaClkIsDefaultClk,           
            kWriteUsingHandshaking => kDmaFifoConfArray(i).InterfaceIsHandshaking)       
          port map (
      ...
      DmaOutput: if kDmaFifoConfArray(i).Mode = NiFpgaHostToTarget or
                    kDmaFifoConfArray(i).Mode = NiFpgaPeerToPeerReader generate
        ...
        assert 
          Log2(kDmaFifoConfArray(i).FifoDepth-6*kDmaFifoConfArray(i).ElementsPerClockCycle+2) > 
          Log2(kDmaFifoConfArray(i).FifoDepth-6*kDmaFifoConfArray(i).ElementsPerClockCycle+1)
          report "kFifoDepth should be 2**N+6*ElementsPerClockCycle-1 where N is an integer."
          severity failure;

        assert kDmaFifoConfArray(i).FifoDepth-6*kDmaFifoConfArray(i).ElementsPerClockCycle+1 >
        kNiDmaDataWidthInBytes
          report "kFifoDepth should be at least the number of samples that fit in two data bus words."
          severity failure;

        assert kDmaFifoConfArray(i).FifoDepth-6*kDmaFifoConfArray(i).ElementsPerClockCycle+1 >
        kDmaFifoConfArray(i).ElementsPerClockCycle
        ...
        --vhook_a kFifoDepth kFifoDepthInDataBusWidthWords(i).FifoDepth
        --vhook_a kSampleWidth kDmaFifoConfArray(i).FifoWidth
        --vhook_a kNumOfSamplesPerRead kDmaFifoConfArray(i).ElementsPerClockCycle
        --vhook_a kScl kDmaFifoConfArray(i).SCL
        --vhook_a kCountScl kDmaFifoConfArray(i).CountSCL
        --vhook_a kFxpType kDmaFifoConfArray(i).FxpType
        --vhook_a kPeerToPeer (kDmaFifoConfArray(i).Mode = NiFpgaPeerToPeerReader)
        --vhook_a kDisableOnFifoTimeout kDmaFifoConfArray(i).DisableOnFifoTimeout
        --vhook_a kViClkIsDefaultClk kDmaFifoConfArray(i).DmaClkIsDefaultClk
        --vhook_a kReadUsingHandshaking kDmaFifoConfArray(i).InterfaceIsHandshaking
        --vhook_a ViClk DmaClkArray(i)
        ...
        DmaPortCommIfcOutputFifoInterfacex: entity work.DmaPortCommIfcOutputFifoInterface (structure)
          generic map (
            kFifoDepth            => kFifoDepthInDataBusWidthWords(i).FifoDepth,         
            kSampleWidth          => kDmaFifoConfArray(i).FifoWidth,                     
            kNumOfSamplesPerRead  => kDmaFifoConfArray(i).ElementsPerClockCycle,         
            kFxpType              => kDmaFifoConfArray(i).FxpType,                       
            kScl                  => kDmaFifoConfArray(i).SCL,                           
            kCountScl             => kDmaFifoConfArray(i).CountSCL,                      
            kPeerToPeer           => (kDmaFifoConfArray(i).Mode = NiFpgaPeerToPeerReader),
            kDisableOnFifoTimeout => kDmaFifoConfArray(i).DisableOnFifoTimeout,          
            kViClkIsDefaultClk    => kDmaFifoConfArray(i).DmaClkIsDefaultClk,            
            kReadUsingHandshaking => kDmaFifoConfArray(i).InterfaceIsHandshaking)        
          port map (
            aDiagramReset                  => aDiagramReset,                             
            aBusReset                      => aBusReset,                                 
            BusClk                         => BusClk,                                    
