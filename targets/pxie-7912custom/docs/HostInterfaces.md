...
This is the host-facing API reference for the PXIe-7912 **custom** target. It
covers every host-accessible register and both DMA FIFOs exposed by the
`UserHdl` block. The source of truth is
[PkgUserHdl.vhd](../rtl-lvfpga/PkgUserHdl.vhd) and
...
| Digital IO (Aux DIO)  | `0x20` | 3  | Direction / OutputData / Status — see [digital-io.md](digital-io.md) |
...
Bit-per-line control of the 8 Aux DIO lines (bit N ↔ line N). Full theory of
operation and usage flows are in [digital-io.md](digital-io.md).
...
(`kNumberOfDmaChannels` comes from the LV Window config
`PkgCommIntConfiguration`; `kNumFixedLogicDmaStreams` from the generated
`PkgNiHdlSettings` — `4` for this FlexRIO target.)

### Using the Writer FIFO (Target-to-Host, DMA 58)

The host reads a data stream out of the FPGA. On this target the FPGA side is
fed by the host itself through the `WriterData` register, so the whole path is
host-driven and self-contained for testing.
...
3. Read the samples back over DMA from stream **58** (e.g.
   `NiFpga_ReadFifoI32`).
4. Observe `WriterCount` (`0x44`) for free space and `WriterState` (`0x4C`) for
   stream state.
5. Stop the stream: write `0x2` to `WriterStartStop`.

### Using the Reader FIFO (Host-to-Target, DMA 59)

The host writes a data stream into the FPGA. On this target the FPGA side is
drained back to the host one element at a time through the `ReaderStrobe` /
`ReaderData` registers.
...
5. Observe `ReaderCount` (`0x48`) for occupancy and `ReaderState` (`0x50`) for
   stream state.
...
`[1:0]` (upper bits read `0`). Use them to confirm the stream reached the
running/stopped state after a start/stop strobe.

## Register access API

All registers are accessed through the standard NI-FPGA register read/write
interface at the offsets listed above (each register is a 32-bit word). Reads of
read-only registers return the FPGA-published value; writes to read-only
registers are ignored. There are no side effects on read except where noted
(`ReaderData` simply returns the last latched value; the pop happens on the
`ReaderStrobe` write).
