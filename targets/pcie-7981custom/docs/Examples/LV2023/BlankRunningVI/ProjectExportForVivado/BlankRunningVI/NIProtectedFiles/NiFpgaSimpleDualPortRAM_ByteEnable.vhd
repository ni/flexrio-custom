-------------------------------------------------------------------------------
--
-- File: NiFpgaSimpleDualPortRAM_ByteEnable.vhd
...
-- of the RAM.
--
--  kDataWidth is the width of the input and output data buses.
--
...
entity NiFpgaSimpleDualPortRAM_ByteEnable is
...
end NiFpgaSimpleDualPortRAM_ByteEnable;


architecture rtl of NiFpgaSimpleDualPortRAM_ByteEnable is
  ...
  type ram_type is array ((2**kAddrWidth)-1 downto 0)
                            of std_logic_vector (kDataWidth-1 downto 0);
  signal iRAM : ram_type := (others => (others => '0'));

  signal oDataOut0, oDataOut1 : std_logic_vector(kDataWidth-1 downto 0) := (others => '0');
  signal oXAddr, oDlyAddr : unsigned(kAddrWidth-1 downto 0) := (others => '0');
...
begin
  ...
  begin
    if rising_edge(IClk) then
      for i in iWrite'range loop		   
        if iWrite(i) then
          iRAM(to_integer(iAddr))((i+1)*8-1 downto i*8) <=
            iDataIn((i+1)*8-1 downto i*8);
        end if;
      end loop;
    end if;
  end process P1;
  ...
  oXAddr <= oDlyAddr when kReadDataLatency=3 else
            oAddr;
  ...
  oDataOut <= oDataOut0 when kReadDataLatency=1 else
              oDataOut1;
						  

  --synthesis translate_off

  -- We have to constrain the data ports width to be a power of two.
  assert kDataWidth rem 8 = 0 
    report "The Data Width is not an integer multiple of 8."
    severity failure;

  -- We have to constrain the data ports width to be greater than one byte.
  assert kDataWidth >= 8
    report "The Data Width is smaller than one byte."
    severity failure;
