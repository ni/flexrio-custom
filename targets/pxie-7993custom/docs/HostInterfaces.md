# PXIe-7994 Custom Target — Host Interfaces (Register & FIFO API)

Host-facing API reference for the PXIe-7994 **custom** target: every
...
| Digital IO (base-board `aBaseDio`) | `0x20` | 3  | Direction / OutputData / Status — see [FlexRIO Digital IO](../../../docs/DigitalIO.md) |
...
|--------|------|--------|-------------|
| `0x00` | Signature | RO | Fixed signature (`0x7994BEEF`) |
...
Bit-per-line control of the **32** base-board DIO lines (bit N ↔ line N). This
target drives the DIO through **separated input / output / output-enable
vectors** (`aBaseDio`) with **no** external direction handshake — see the common
...
| `0x28` | Status     | RO  | `[31:0]` live input per line |

> These three registers are a **demonstration** of the DIO capability, not the
> intended use case — the real interface is the `aBaseDio` in/out/output-enable
> vectors in your custom HDL. See Part 2 of the common
> [FlexRIO Digital IO](../../../docs/DigitalIO.md) document.
