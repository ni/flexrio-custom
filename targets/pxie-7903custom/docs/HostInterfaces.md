# PXIe-7903 Custom Target — Host Interfaces (Register & FIFO API)
...
| Digital IO (base-board `aDio`) | `0x20` | 3  | Direction / OutputData / Status — see [FlexRIO Digital IO](../../../docs/DigitalIO.md) |
...
Bit-per-line control of the **8** base-board DIO lines (bit N ↔ line N). This
target drives the DIO through a **direct bidirectional bus** (`aDio`) with **no**
external direction handshake — see the common
...
> intended use case — the real interface is the `aDio` bidirectional bus in your
